...
function BearSwitch() {
  ...
  function handleUncheck() {
    // Show arm then it should feel like arm is pulling the switch back and then switch back to unchecked
    setShowArm(true)
  ...
  return (
    <div className='flex justify-center relative'>
      <motion.div
        className='h-10 w-10 absolute z-[-10]'
        variants={{
          show: {
            scale: 1.5,
            rotate: 23,
        ...
        initial={checked ? 'show' : 'hide'}
        animate={checked ? 'show' : hovered ? 'hover' : 'hide'}
      ...
      >
        <div className='relative'>
          ...
          {(checked || hovered) && count > angerLimit + 1 && (
            <div className='top-2 absolute left-[105%] rounded-full bg-gray-300 p-1'>
              <p className='text-[8px] text-gray-900 text-nowrap'>
                Leave me alone!!
              </p>
            </div>
          )}
      ...
      <motion.div
        ...
        initial={showArm ? 'showArm' : 'hideArm'}
        animate={showArm ? 'showArm' : 'hideArm'}
        transition={{ duration: 0.4 }}
      >
        <svg
          className=''
      ...
      <motion.button
        className={cn('rounded-full p-2 h-20 w-40')}
        onClick={() => {
          if (checked) {
            handleUncheck()
          } else {
      ...
      >
        <motion.div
          className={cn(`h-full aspect-square bg-white rounded-full shadow-md`)}
          variants={{
            checked: { translateX: 80 },
